...
log_file = log_path / (
    f'groq_whisperer_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
)
...
def record_audio(sample_rate=16000, channels=1, chunk=1024):
    ...
    msg = (
        "[yellow]Press and hold the alt+x button to start recording...[/yellow]"
    )
...
def transcribe_audio(audio_file_path):
    ...
    try:
        ...
        logger.error(f"Transcription error: {str(e)}", exception=e)
...
def main():
    ...
    layout["header"].update(
        Align.center("[bold cyan]Groq Whisperer[/bold cyan]")
    )
    layout["footer"].update(
        Align.center("[yellow]Press alt+x to start recording[/yellow]")
    )
    layout["status"].update(Align.center(""))
        ...
        with Live(layout, refresh_per_second=4, screen=False) as live:
            while True:
                try:
                    ...
                    logger.debug(
                        f"Audio recorded: {len(frames)} frames at {sample_rate}Hz"
                    )
                    ...
                    if transcription:
                        ...
                        layout["status"].update(
                            Align.center("[red]Transcription failed[/red]")
                        )
                        live.refresh()
